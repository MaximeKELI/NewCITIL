...
echo "🚀 DÉMARRAGE STABLE DE LA PLATEFORME CITIL"
echo "=========================================="
...
# Configuration PHP optimisée pour éviter les crashes
...
php -d memory_limit=1024M \
    ...
    artisan serve --host=0.0.0.0 --port=8002 &
BACKEND_PID=$!
...
if curl -s http://localhost:8002/api/test > /dev/null; then
    ...
    sleep 2
    php -d memory_limit=1024M -d max_execution_time=0 artisan serve --host=0.0.0.0 --port=8002 &
    BACKEND_PID=$!
    sleep 5
fi
